...
const Renderer = (() => {
  ...
  async function loadColorizedImage(assetUrl, color, tintColor) {
    ...
    // PNG/JPG: load directly (no colorization via text replace)
    if (!assetUrl.endsWith('.svg')) {
      return loadImageFromUrl(assetUrl);
    }

    // SVG: fetch as text, colorize, convert to data URL
  ...
  async function renderToDataURL(charData, maxHeight = 140) {
    ...
    const scale = maxHeight / shape.dimensions.height;
    const stage = await renderToStage(offscreen.id, charData, scale);
    if (!stage) {
      offscreen.remove();
      return null;
    }
